...
/* Instances of classmethod_descriptor are very unlikely to be called
   directly. For one, the analogous class "classmethod" is not callable.
   Second, users are not likely to access a classmethod_descriptor
   directly, since it means pulling it from the class __dict__.

   This is all an excuse to say that this doesn't need to be
                 ...
                 PyObject *kwds)
{
    ...
    PyObject *res = PyCCall_FASTCALL(bound, &PyTuple_GET_ITEM(args, 1), argc-1, kwds);
    Py_DECREF(bound);
    return res;
}
