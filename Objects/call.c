...
_PyObject_HasFastCall(PyObject *callable)
{
    if (PyFunction_Check(callable)) {
        ...
        /* A C call instance is said to support FASTCALL if no temporary
           ...
           case for CCALL_FASTCALL, CCALL_NULLARG and CCALL_O but not
           for CCALL_VARARGS. */
        uint32_t flags = PyCCall_CCALLDEF(callable)->cc_flags;
...
ccall_nargs_error(PyCCallDef *cc, Py_ssize_t nargs)
{
    switch (cc->cc_flags & CCALL_BASESIGNATURE)
    {
    case CCALL_NULLARG:
    ...
    default:
         if (nargs == 0) {
            ...
            /* This case doesn't actually happen */
            PyErr_Format(PyExc_TypeError,
                         "%U() called with invalid number of arguments (%zd given)",
                         cc->cc_name, nargs);
        }
    }
}
...
{
    PyCCallDef *cc = PyCCall_CCALLDEF(func);
    PyCFunc cfunc = cc->cc_func;

    switch (cc->cc_flags & CCALL_SIGNATURE) {
    case CCALL_VARARGS:
        return ((PyCFunc_SA)cfunc)(self, args);
    case CCALL_VARARGS | CCALL_FUNCARG:
        return ((PyCFunc_FSA)cfunc)(func, self, args);
    case CCALL_VARARGS | CCALL_KEYWORDS:
        return ((PyCFunc_SAK)cfunc)(self, args, kwargs);
    case CCALL_VARARGS | CCALL_KEYWORDS | CCALL_FUNCARG:
        return ((PyCFunc_FSAK)cfunc)(func, self, args, kwargs);
    default:
        PyErr_Format(PyExc_SystemError,
                     "%.200s() has invalid flags", cc->cc_name);
        return NULL;
    }
}
                 ...
                 PyObject *keywords)
{
    ...
    PyCCallRoot *cr = PyCCall_CCALLROOT(func);
    PyCCallDef *cc = cr->cr_ccall;
    PyObject *self = cr->cr_self;
    uint32_t flags = cc->cc_flags;
    PyCFunc cfunc = cc->cc_func;

    if (self == NULL && (flags & (CCALL_OBJCLASS | CCALL_SLICE_SELF)))
    {
        /* Check __objclass__ and do self slicing */
        if (nargs == 0) {
            ccall_nargs_error(cc, nargs);
            return NULL;
        }
    ...
    if (keywords == NULL) {
        /* Case 1: no keyword arguments are passed (or by the above
           ...
           This is the case that is most important to optimize.
           Note that the C function may or may not accept keywords. */
        switch (flags & CCALL_SIGNATURE)
        {
        case CCALL_FASTCALL:
            result = ((PyCFunc_SNn)cfunc)(self, args, nargs);
            break;
        case CCALL_FASTCALL | CCALL_FUNCARG:
            result = ((PyCFunc_FSNn)cfunc)(func, self, args, nargs);
            break;
        case CCALL_FASTCALL | CCALL_KEYWORDS:
            result = ((PyCFunc_SNnK)cfunc)(self, args, nargs, NULL);
            break;
        case CCALL_FASTCALL | CCALL_KEYWORDS | CCALL_FUNCARG:
            result = ((PyCFunc_FSNnK)cfunc)(func, self, args, nargs, NULL);
            break;
        case CCALL_NULLARG:
            if (nargs != 0) {
                ccall_nargs_error(cc, nargs);
            ...
            break;
        case CCALL_NULLARG | CCALL_FUNCARG:
            if (nargs != 0) {
                ccall_nargs_error(cc, nargs);
                break;
            }
            result = ((PyCFunc_FSA)cfunc)(func, self, NULL);
            break;
        case CCALL_O:
            if (nargs != 1) {
                ccall_nargs_error(cc, nargs);
            ...
            break;
        case CCALL_O | CCALL_FUNCARG:
            if (nargs != 1) {
                ccall_nargs_error(cc, nargs);
                break;
            }
            result = ((PyCFunc_FSA)cfunc)(func, self, args[0]);
            break;
        default:
        {
        ...
        switch (flags & (CCALL_SIGNATURE & ~CCALL_FUNCARG))
        {
        case CCALL_FASTCALL | CCALL_KEYWORDS:
        {
            ...
            if (flags & CCALL_FUNCARG) {
                result = ((PyCFunc_FSNnK)cfunc)(func, self, stack, nargs, kwnames);
            }
            else {
                result = ((PyCFunc_SNnK)cfunc)(self, stack, nargs, kwnames);
            }
            if (stack != args) {
        ...
        default:
            PyErr_Format(PyExc_TypeError,
                         "%U() takes no keyword arguments", cc->cc_name);
        }
    }
...
PyCCall_Call(PyObject *func, PyObject *args, PyObject *kwargs)
{
    ...
    if ((flags & CCALL_BASESIGNATURE) == CCALL_VARARGS) {
        PyObject *self = cr->cr_self;
        if ((self != NULL) ||
            (flags & (CCALL_OBJCLASS | CCALL_SLICE_SELF)) == 0)
        {
            if (kwargs) {
...
PyObject *
PyCCall_GenericGetSelf(PyObject *func, void *closure)
{
    if (!PyCCall_Check(func)) {
        PyErr_BadInternalCall();
        return NULL;
    }
    PyObject *attr = PyCCall_SELF(func);

    if (attr == NULL) {
        PyErr_Format(PyExc_AttributeError,
                     "'%.100s' object has no attribute '%s'",
                     Py_TYPE(func)->tp_name,
                     closure == NULL ? "__self__" : (char*)closure);
        return NULL;
    }
    Py_INCREF(attr);
    return attr;
...
PyObject *
PyCCall_GenericGetName(PyObject *func, void *closure)
{
    if (!PyCCall_Check(func)) {
        PyErr_BadInternalCall();
        return NULL;
    }
    PyObject *attr = PyCCall_CCALLDEF(func)->cc_name;
    Py_INCREF(attr);
    return attr;
}
